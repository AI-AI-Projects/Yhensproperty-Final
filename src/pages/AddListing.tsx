...
const AddListing: React.FC<AddListingProps> = ({ onAdd, isEdit }) => {
  ...
  const handleImageUpload = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (files && files.length > 0) {
      ...
      addErrorLog(`📤 Starting upload: ${files.length} file(s) | Device: ${deviceInfo}`);

      Array.from(files).forEach(async (file: File, index: number) => {
        try {
          addErrorLog(`Processing ${file.name} (${(file.size / 1024 / 1024).toFixed(2)}MB)`);

          const originalSize = await getFileSizeInMB(file);
          addErrorLog(`Original size: ${originalSize.toFixed(2)}MB`);

          addErrorLog('Starting compression...');
          const compressedBlob = await compressImage(file);
          const compressedSize = await getFileSizeInMB(compressedBlob);
          addErrorLog(`Compressed: ${compressedSize.toFixed(2)}MB`);

          addErrorLog('Converting to base64...');
          const base64 = await fileToBase64(compressedBlob);
          addErrorLog(`✅ Success: ${file.name}`);
          ...
          addErrorLog(`❌ FAILED: ${file.name} - ${errorMsg}`);
          setShowErrorLog(true);
          alert(`Failed to process "${file.name}"\n\nError: ${errorMsg}\n\nClick "View Debug Log" button below to see full details.`);
        }
      });
    }
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    addErrorLog(`🚀 Starting submission | Device: ${deviceInfo.platform}`);
    ...
    try {
      ...
      addErrorLog(`📸 Uploading ${formData.images.length} images to storage...`);
      for (let i = 0; i < formData.images.length; i++) {
        const imageBase64 = formData.images[i];

        if (imageBase64.startsWith('http')) {
          imageUrls.push(imageBase64);
          addErrorLog(`✓ Image ${i + 1} already uploaded`);
          continue;
        }

        try {
          addErrorLog(`Converting image ${i + 1}...`);
          ...
          addErrorLog(`Blob created: ${(blob.size / 1024).toFixed(2)}KB`);

          const fileName = `${propertyId}_${Date.now()}_${i}.webp`;
          addErrorLog(`Uploading ${fileName}...`);
          ...
          if (uploadError) {
            addErrorLog(`❌ Upload failed: ${uploadError.message}`);
            throw uploadError;
          }
          ...
          addErrorLog(`✓ Image ${i + 1} uploaded`);
        } catch (imgError) {
          const errorMsg = imgError instanceof Error ? imgError.message : String(imgError);
          addErrorLog(`❌ Image ${i + 1} failed: ${errorMsg}`);
          throw new Error(`Failed to process image ${i + 1}: ${errorMsg}`);
        }
      }

      addErrorLog('💾 Saving listing to database...');
      ...
      addErrorLog('✅ Listing saved successfully!');
      setShowSuccess(true);
      ...
      addErrorLog(`❌ SUBMISSION FAILED: ${errorMsg}`);
      setShowErrorLog(true);

      let errorMessage = "There was an error saving your listing.";
      errorMessage += `\n\nError: ${errorMsg}`;
  ...
  return (
    <div className="max-w-7xl mx-auto px-6 py-12 relative">
      ...
      {/* Debug Log Modal */}
      {showErrorLog && (
        <div className="fixed inset-0 z-[200] flex items-center justify-center bg-zinc-900/80 backdrop-blur-md p-4" onClick={() => setShowErrorLog(false)}>
          <div className="bg-white dark:bg-zinc-800 rounded-2xl shadow-2xl max-w-2xl w-full max-h-[80vh] overflow-hidden flex flex-col" onClick={e => e.stopPropagation()}>
            <div className="flex items-center justify-between p-6 border-b border-zinc-200 dark:border-zinc-700">
              <div>
                ...
                <p className="text-sm text-zinc-500 mt-1">Technical details for troubleshooting</p>
              </div>
              <button onClick={() => setShowErrorLog(false)} className="w-10 h-10 flex items-center justify-center rounded-full hover:bg-zinc-100 dark:hover:bg-zinc-700 transition-colors">
                <span className="material-icons">close</span>
              </button>
            </div>
            <div className="flex-1 overflow-y-auto p-6 font-mono text-xs bg-zinc-50 dark:bg-zinc-900">
              {errorLog.map((log, i) => (
                <div key={i} className={`py-1 ${log.includes('❌') ? 'text-red-600 dark:text-red-400 font-bold' : log.includes('✅') ? 'text-green-600 dark:text-green-400' : 'text-zinc-600 dark:text-zinc-400'}`}>
                  {log}
                </div>
              ))}
            </div>
            <div className="p-4 border-t border-zinc-200 dark:border-zinc-700 flex gap-2">
              <button
                ...
                className="flex-1 py-3 bg-zinc-100 dark:bg-zinc-700 text-zinc-900 dark:text-white rounded-xl font-bold text-sm hover:bg-zinc-200 dark:hover:bg-zinc-600 transition-colors flex items-center justify-center gap-2"
              >
              ...
              <button
                ...
                className="flex-1 py-3 bg-primary text-zinc-900 rounded-xl font-bold text-sm hover:brightness-110 transition-all flex items-center justify-center gap-2"
              >
                Close
              </button>
            </div>
      ...
      <div className="flex items-center gap-4 mb-8">
        ...
        {errorLog.length > 0 && (
          <button
            ...
            className="px-4 py-2 bg-zinc-100 dark:bg-zinc-800 border border-zinc-200 dark:border-zinc-700 rounded-xl text-sm font-bold hover:bg-zinc-200 dark:hover:bg-zinc-700 transition-colors flex items-center gap-2"
          >
                  ...
                  {!isLand && (
                    <>
                      <div className="flex flex-col gap-2">
                        ...
                        <select
                          name="beds"
                          value={formData.beds === 0 ? 'STUDIO' : formData.beds || ''}
                          onChange={(e) => {
                            const value = e.target.value === 'STUDIO' ? 0 : parseInt(e.target.value);
                            handleChange({ target: { name: 'beds', value: value } } as any);
                          }}
                          className="w-full bg-zinc-50 dark:bg-zinc-800 border border-zinc-200 dark:border-zinc-700 rounded-xl px-4 py-2.5 focus:ring-2 focus:ring-primary focus:border-primary dark:text-white"
                          required={!isCommercial && !isWarehouse}
                        >
                  ...
                  <div className="flex flex-col">
                    ...
                    {formData.condoName && (
                      <span className="text-[10px] text-primary font-semibold mt-1">
                        {formData.condoName}{isCondo && formData.floor ? ` - Floor ${formData.floor}` : ''}
                      </span>
                    )}
                  </div>
                </div>
