      ...
      <div className="p-6 flex flex-col flex-grow">
            ...
            {showShareMenu && (
              <div className="absolute right-0 top-10 z-50 bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-700 rounded-2xl shadow-2xl w-44 animate-in fade-in slide-in-from-top-2 duration-150 overflow-hidden">
                ...
                <a
                  ...
                  onClick={(e) => { e.stopPropagation(); setShowShareMenu(false); window.dataLayer = window.dataLayer || []; window.dataLayer.push({ event: 'whatsapp_click', button_location: 'property_card_share' }); }}
                  className="w-full flex items-center gap-3 px-4 py-3 text-sm font-bold text-zinc-700 dark:text-zinc-200 hover:bg-zinc-50 dark:hover:bg-zinc-800 transition-colors"
                >
        ...
        <div className="flex gap-2 pt-4 mt-4 border-t border-zinc-100 dark:border-zinc-800">
          <a
            ...
            target="_blank"
            rel="noopener noreferrer"
            onClick={(e) => { e.stopPropagation(); window.dataLayer = window.dataLayer || []; window.dataLayer.push({ event: 'whatsapp_click', button_location: 'property_card' }); }}
            className="whatsapp-conversion flex-1 flex items-center justify-center gap-2 bg-[#25D366] hover:bg-[#20BA5A] text-white font-bold py-2.5 px-4 rounded-xl transition-all shadow-sm hover:shadow-md active:scale-95"
          >
...
const Home: React.FC<HomeProps> = ({ properties, isLoading }) => {
  ...
  const homeStructuredData = {
    ...
    "alternateName": "Yhens Property",
    "description": "Licensed real estate broker specializing in luxury condos, houses, land and commercial properties across the Philippines",
    ...
    "knowsAbout": ["Condominiums", "Houses", "Villas", "Land", "Commercial Properties", "Real Estate", "Properties for Sale", "Properties for Rent"]
  };

  return (
    <div>
      <SEO
        title="Yhens Property | Yhen's Property – Luxury Condos & Houses in Philippines for Locals & Foreigners | BGC, Manila, Batangas"
        description="Yhens Property (Yhen's Property) is a licensed agent helping Filipinos and foreigners buy or rent condos, houses, land and commercial properties across the Philippines. Direct deals, legal guidance for expats, focus on BGC, Manila and Batangas"
        canonical="https://yhensproperty.com"
        ogType="website"
        ogTitle="Yhens Property - Premium Real Estate"
        ogDescription="Yhens Property (Yhen's Property) is a licensed real estate broker offering luxury condos, houses, land and commercial properties for sale and rent."
        ogUrl="https://yhensproperty.com"
        ogImage="https://yhensproperty.com/Image/Yhen_Property_Favikan.png"
        ogSiteName="Yhens Property"
        ogLocale="en_PH"
        twitterCard="summary_large_image"
        twitterTitle="Yhens Property"
        twitterDescription="Luxury condos, houses, land and commercial properties in the Philippines"
        twitterImage="https://yhensproperty.com/Image/Yhen_Property_Favikan.png"
            ...
            <form onSubmit={handleSearch} className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-6 lg:grid-cols-12 gap-2.5 md:gap-3 items-stretch">
              ...
              <div className="md:col-span-2 lg:col-span-2 relative">
                ...
                >
                  ...
                  <option value="0" className="bg-zinc-900">Studio</option>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-8">
          {[
            ...
            <Link key={i} to={`/category/${cat.slug}`} className="group relative h-96 rounded-[40px] overflow-hidden cursor-pointer block border border-zinc-100 dark:border-zinc-800 shadow-2xl">
              {cat.img && <img src={cat.img} className="w-full h-full object-cover transition-transform duration-1000 group-hover:scale-110" alt={`${cat.label} category - Browse properties`} loading="lazy" decoding="async" width="800" height="600" />}
              <div className="absolute inset-0 bg-gradient-to-t from-black/90 via-black/40 to-transparent flex flex-col justify-end p-8">
                <div className="flex items-center gap-4">
                  <div className="bg-primary p-3 rounded-2xl text-zinc-900 shadow-xl shadow-primary/30 group-hover:rotate-12 transition-transform duration-500 flex-shrink-0">
                    <span className="material-icons text-2xl">{cat.icon}</span>
                  </div>
                  <h3 className="text-2xl font-black text-white tracking-tight leading-tight whitespace-pre-line">{cat.label}</h3>
                </div>
              </div>
            </Link>
          ))}
      ...
      {/* Featured Investor Guides */}
      <section className="py-20 bg-zinc-950">
        <div className="max-w-7xl mx-auto px-6">
          <div className="flex flex-col sm:flex-row sm:items-end justify-between gap-6 mb-12">
            <div>
              <span className="text-primary font-bold tracking-[0.4em] text-[10px] mb-3 block uppercase">Knowledge Hub</span>
              <h2 className="text-3xl md:text-4xl font-black text-white tracking-tighter leading-tight">
                Investor Guides for<br className="hidden sm:block" /> International Buyers
              </h2>
              <p className="text-zinc-400 text-sm mt-3 max-w-md">
                Expert guides for Americans, Canadians, Australians &amp; Europeans buying, selling, renting, or investing in Philippine real estate.
              </p>
            </div>
            <Link
              to="/guides"
              className="shrink-0 flex items-center gap-2 text-sm font-bold text-primary border border-primary/30 hover:bg-primary/10 px-5 py-2.5 rounded-xl transition-colors"
            >
              View All Guides
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-5">
            {guides.slice(0, 3).map((guide) => (
              <Link
                ...
                className="group flex flex-col bg-white/5 hover:bg-white/10 border border-white/10 hover:border-primary/30 rounded-2xl p-6 transition-all duration-300"
              >
                <div className="flex items-center gap-3 mb-4">
                  <div className="bg-primary/20 group-hover:bg-primary text-primary group-hover:text-zinc-900 p-2.5 rounded-xl transition-colors">
                    <span className="material-icons text-lg">{guide.icon}</span>
                  </div>
                  <div>
                    <span className="text-[9px] font-black uppercase tracking-[0.3em] text-zinc-500">{guide.badge}</span>
                    <p className="text-[10px] text-zinc-500 flex items-center gap-1">
                      <span className="material-icons text-xs">schedule</span>{guide.readTime}
                    </p>
                  </div>
                </div>
                <h3 className="font-black text-white text-sm leading-tight mb-3 group-hover:text-primary transition-colors">
                  {guide.title}
                </h3>
                ...
                <span className="text-xs font-bold text-primary flex items-center gap-1 group-hover:gap-2 transition-all">
                  Read Guide <span className="material-icons text-xs">arrow_forward</span>
                </span>
              </Link>
            ))}
          </div>
          <div className="mt-8 text-center">
            <Link
              to="/guides"
              className="inline-flex items-center gap-2 bg-primary hover:brightness-110 text-zinc-900 font-bold px-8 py-4 rounded-2xl transition-all shadow-lg shadow-primary/20 hover:scale-105"
            >
              <span className="material-icons text-sm">menu_book</span>
              Explore All Investor Guides
            </Link>
          </div>
        </div>
      </section>
