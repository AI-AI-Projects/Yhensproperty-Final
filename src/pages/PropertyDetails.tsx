...
const PropertyDetails: React.FC<PropertyDetailsProps> = ({ properties, isLoading }) => {
  ...
  useEffect(() => {
    ...
    const observer = new IntersectionObserver(
      ([entry]) => { if (entry.isIntersecting) { setCaptchaVisible(true); observer.disconnect(); } },
      { rootMargin: '200px' }
    );
  ...
  if (isLoading) {
    return (
      <div className="max-w-7xl mx-auto px-6 py-6 animate-pulse">
        <div className="mb-6">
          <div className="h-3 w-40 bg-zinc-200 dark:bg-zinc-800 rounded mb-4"></div>
          <div className="h-10 w-3/4 bg-zinc-200 dark:bg-zinc-800 rounded-xl mb-3"></div>
          <div className="h-4 w-48 bg-zinc-200 dark:bg-zinc-800 rounded mb-2"></div>
        </div>
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
          <div className="lg:col-span-8 space-y-6">
            <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-2 sm:gap-3 h-auto lg:h-[400px]">
              <div className="col-span-2 lg:row-span-2 bg-zinc-200 dark:bg-zinc-800 rounded-2xl h-64 sm:h-80 lg:h-full"></div>
              <div className="bg-zinc-200 dark:bg-zinc-800 rounded-xl h-32 sm:h-40 lg:h-auto"></div>
              <div className="bg-zinc-200 dark:bg-zinc-800 rounded-xl h-32 sm:h-40 lg:h-auto"></div>
              <div className="bg-zinc-200 dark:bg-zinc-800 rounded-xl h-32 sm:h-40 lg:h-auto"></div>
              <div className="bg-zinc-200 dark:bg-zinc-800 rounded-xl h-32 sm:h-40 lg:h-auto"></div>
            </div>
            <div className="bg-zinc-100 dark:bg-zinc-900/50 rounded-3xl p-6 border border-zinc-100 dark:border-zinc-800">
              <div className="flex flex-wrap gap-6">
                <div className="flex-1 min-w-[200px]">
                  <div className="h-3 w-24 bg-zinc-200 dark:bg-zinc-700 rounded mb-3"></div>
                  <div className="h-10 w-40 bg-zinc-200 dark:bg-zinc-700 rounded-lg"></div>
                </div>
                <div className="flex gap-8">
                  {[1,2,3,4].map(i => (
                    <div key={i} className="text-center space-y-2">
                      <div className="w-6 h-6 bg-zinc-200 dark:bg-zinc-700 rounded mx-auto"></div>
                      <div className="h-5 w-8 bg-zinc-200 dark:bg-zinc-700 rounded mx-auto"></div>
                      <div className="h-2 w-10 bg-zinc-200 dark:bg-zinc-700 rounded mx-auto"></div>
                    </div>
                  ))}
                </div>
              </div>
            ...
            <div>
              <div className="h-7 w-48 bg-zinc-200 dark:bg-zinc-800 rounded-lg mb-4"></div>
          ...
          <div className="lg:col-span-4">
            <div className="bg-white dark:bg-zinc-900 p-6 rounded-3xl border border-zinc-200 dark:border-zinc-800 space-y-4">
              <div className="flex items-center gap-4">
                ...
                <div className="space-y-2">
                  <div className="h-5 w-20 bg-zinc-200 dark:bg-zinc-700 rounded"></div>
              ...
              <div className="h-11 bg-zinc-200 dark:bg-zinc-700 rounded-xl mt-2"></div>
              <div className="h-11 bg-zinc-200 dark:bg-zinc-700 rounded-xl"></div>
            </div>
          </div>
        </div>
      </div>
  ...
  const generateSEODescription = (): string => {
    ...
    if (!isLand) {
      const bedsText = property.beds === 0 ? 'Studio' : `${property.beds} bed${property.beds > 1 ? 's' : ''}`;
      desc += ` • ${bedsText}, ${property.baths} bath${property.baths > 1 ? 's' : ''}`;
    }
  ...
  const generateImageAlt = (imageIndex: number): string => {
    ...
    if (imageIndex === 0) {
      ...
      if (!isLand) {
        const bedsText = property.beds === 0 ? 'Studio' : `${property.beds} bedroom`;
        altText += ` - ${bedsText}, ${property.baths} bathroom`;
      }
      if (property.condoName) {
  ...
  const imageUrl = featuredImageUrl
    ...
    : `${window.location.origin}/Image/Hero_Villa_2.webp`;
    ...
    <>
      ...
      {showSuccess && (
        <div className="fixed inset-0 z-[120] flex items-center justify-center bg-zinc-950/90 backdrop-blur-xl animate-in fade-in duration-300">
          <div id="form-success-message" className="bg-white dark:bg-zinc-900 p-12 rounded-[40px] shadow-2xl text-center flex flex-col items-center gap-6 animate-in zoom-in-95 duration-500">
            ...
            <div className="relative flex-shrink-0" ref={shareRef}>
              ...
              {showShareMenu && (
                <div className="absolute right-0 md:left-0 top-12 z-50 bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-700 rounded-2xl shadow-2xl overflow-hidden w-48 animate-in fade-in slide-in-from-top-2 duration-150">
                  ...
                  <div className="h-px bg-zinc-100 dark:bg-zinc-800 mx-3"></div>
                  <a
                    href={`https://wa.me/639467543767?text=${encodeURIComponent(`Check out this property: "${property?.title}" in ${property?.city}\n${window.location.href}`)}`}
                    target="_blank"
                    rel="noopener noreferrer"
                    onClick={() => { setShowShareMenu(false); window.dataLayer = window.dataLayer || []; window.dataLayer.push({ event: 'whatsapp_click', button_location: 'property_details_share' }); }}
                    className="w-full flex items-center gap-3 px-4 py-3.5 text-sm font-bold text-zinc-700 dark:text-zinc-200 hover:bg-zinc-50 dark:hover:bg-zinc-800 transition-colors"
                  >
          ...
          {property.condoName && (
            <div className="flex items-center gap-2 text-primary font-semibold">
              ...
              <span className="text-sm">
                {property.condoName}
                {(property.type === PropertyType.Condo || property.type === PropertyType.Apartment) && property.floor && ` - Floor ${property.floor}`}
              </span>
            </div>
          )}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
        <div className="lg:col-span-8 space-y-8">
          ...
          <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-2 sm:gap-3 h-auto lg:h-[400px] relative">
            ...
            >
              <img className="w-full h-full object-cover transition-transform duration-700 group-hover:scale-105" src={property.images[property.featuredImageIndex ?? 0] ?? property.images[0]} alt={generateImageAlt(property.featuredImageIndex ?? 0)} loading="eager" fetchPriority="high" decoding="sync" width="1200" height="800" />
            </div>
            {property.images.slice(1, 5).map((img, idx) => (
              ...
              >
                <img className="w-full h-full object-cover" src={img} alt={generateImageAlt(idx + 1)} loading="lazy" decoding="async" width="800" height="600" />
              </div>
            ))}
          </div>
          ...
          <section className="bg-zinc-50 dark:bg-zinc-900/50 p-6 rounded-3xl border border-zinc-100 dark:border-zinc-800 shadow-sm">
            <div className="flex flex-wrap items-center justify-between gap-6">
              ...
              <div className={`flex-grow grid ${isLand ? 'grid-cols-3' : (property.type === PropertyType.Condo || property.type === PropertyType.Apartment) && property.floor ? 'grid-cols-3 md:grid-cols-6' : 'grid-cols-3 md:grid-cols-5'} gap-4 items-center`}>
                {!isLand && (
                  <>
                    <div className="text-center">
                      <span className="material-icons text-primary text-xl block mb-0.5">king_bed</span>
                      <span className="block font-black text-lg dark:text-white leading-none">{property.beds === 0 ? 'Studio' : property.beds}</span>
                      <span className="text-[9px] text-zinc-400 uppercase font-black tracking-widest">{property.beds === 0 ? 'Type' : 'Beds'}</span>
                    </div>
                    <div className="text-center">
                    ...
                    {(property.type === PropertyType.Condo || property.type === PropertyType.Apartment) && property.floor && (
                      <div className="text-center">
                        <span className="material-icons text-primary text-xl block mb-0.5">layers</span>
                        <span className="block font-black text-lg dark:text-white leading-none">{property.floor}</span>
                        <span className="text-[9px] text-zinc-400 uppercase font-black tracking-widest">Floor</span>
                      </div>
                    )}
                  </>
                )}
        ...
        {/* Sidebar / Form */}
        <div className="lg:col-span-4 relative" ref={formRef}>
            ...
            <div className="bg-white dark:bg-zinc-900 p-6 rounded-3xl shadow-2xl border border-zinc-200 dark:border-zinc-800 ring-1 ring-black/5">
              ...
              <form className="space-y-4" onSubmit={handleInquirySubmit}>
                ...
                <div className="flex justify-center py-2 min-h-[78px]">
                  {captchaVisible && (
                    <Suspense fallback={<div className="h-[78px] w-[303px] bg-zinc-100 dark:bg-zinc-800 rounded-lg animate-pulse" />}>
                      <HCaptcha
                        ref={captchaRef}
                ...
                <div className="flex flex-col gap-3 pt-1">
                  ...
                  <a
                    ...
                    href={`https://wa.me/639467543767?text=${encodeURIComponent(`Hi Yhen, I'm interested in "${property.title}" located in ${property.city}. \n\nCheck out the property details here: ${window.location.href}`)}`}
                    target="_blank"
                    rel="noopener noreferrer"
                    className="whatsapp-conversion w-full bg-[#25D366] text-white font-black py-3.5 rounded-xl shadow-xl hover:brightness-110 active:scale-[0.98] transition-all flex items-center justify-center gap-2.5"
                    onClick={() => { window.dataLayer = window.dataLayer || []; window.dataLayer.push({ event: 'whatsapp_click', button_location: 'property_details_sidebar' }); }}
                  >
                  ...
                  <a
                    id="messenger-conversion"
                    href="https://m.me/yhensproperty"
                    target="_blank"
                    rel="noopener noreferrer"
                ...
                >
                  <div className="relative h-40 sm:h-48 overflow-hidden">
                    <img
                      ...
                      alt={`${prop.type} ${prop.listingType === 'sale' ? 'for sale' : 'for rent'} in ${prop.city}${!isLand && prop.beds !== undefined ? ` - ${prop.beds === 0 ? 'Studio' : `${prop.beds} bedroom`}, ${prop.baths} bathroom` : ''}${prop.condoName ? ` at ${prop.condoName}` : ''}`}
                      ...
                      {prop.type !== PropertyType.Land && (
                        <>
                          <div className="flex items-center gap-1">
                            ...
                            <span>{prop.beds === 0 ? 'Studio' : prop.beds}</span>
                          </div>
                          <div className="flex items-center gap-1">
