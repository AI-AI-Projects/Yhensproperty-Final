...
const CommissionList: React.FC<CommissionListProps> = ({ commissions, properties, onManageCommission }) => {
  ...
  const getAgentFields = (propertyId: string) => {
    const agent = getPropertyAgent(propertyId);
    return {
      showDaphne: agent === 'Daphne',
      showAbby: agent === 'Abby',
      showJuvy: agent === 'Juvy',
    };
  };
        ...
        <>
          <div className="block md:hidden divide-y divide-zinc-100 dark:divide-zinc-800">
            {currentItems.map((commission) => {
              const { showDaphne, showAbby, showJuvy } = getAgentFields(commission.property_id);
              return (
                  ...
                  return (
                    ...
                    >
                      ...
                      <td className="px-4 lg:px-8 py-4 lg:py-5">
                        {showDaphne && (
                          <div className="space-y-1">
                            <p className="text-xs text-zinc-500 font-bold mb-1">Daphne</p>
                            {getPaymentStatusBadge(commission.daphne_paid || false)}
                            <p className="text-xs text-zinc-500">
                        ...
                        {showAbby && (
                          <div className="space-y-1">
                            <p className="text-xs text-zinc-500 font-bold mb-1">Abby</p>
                            {getPaymentStatusBadge(commission.abby_paid || false)}
                            <p className="text-xs text-zinc-500">
                        ...
                        {showJuvy && (
                          <div className="space-y-1">
                            <p className="text-xs text-zinc-500 font-bold mb-1">Juvy</p>
                            {getPaymentStatusBadge(commission.juvy_paid || false)}
                            <p className="text-xs text-zinc-500">
