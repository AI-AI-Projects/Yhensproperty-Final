...
const Navbar: React.FC = () => {
  ...
  const logoUrl = "/Image/LimeLogo2.webp";
  const whatsappNumber = siteConfig.contact.whatsapp;
  const facebookUrl = siteConfig.contact.facebook;
    ...
    <nav className="sticky top-0 z-50 bg-white dark:bg-zinc-900 border-b border-zinc-200 dark:border-zinc-800 px-6 py-4">
      <div className="max-w-7xl mx-auto flex items-center justify-between">
        <Link to="/" className="flex items-center group" onClick={handleLogoClick}>
          <img src={logoUrl} alt={`${siteConfig.searchName} Logo`} width="195" height="40" className="h-10 w-auto object-contain group-hover:scale-105 transition-transform duration-300 rounded-lg" style={{ aspectRatio: '823/169' }} />
        </Link>
        ...
        <div className="hidden md:flex items-center space-x-6 lg:space-x-8">
          ...
          <div className="relative group h-full flex items-center"
            ...
            <button className="flex items-center gap-1 text-sm font-medium hover:text-primary transition-colors py-2">
              Resources
              <span className={`material-icons text-xs transition-transform duration-200 ${isGuidesOpen ? 'rotate-180' : ''}`}>expand_more</span>
            </button>
            <div className={`absolute top-full left-0 w-56 pt-2 transition-all duration-200 origin-top-left ${isGuidesOpen ? 'opacity-100 scale-100 translate-y-0 visible' : 'opacity-0 scale-95 -translate-y-2 invisible'}`}>
              <div className="bg-white dark:bg-zinc-900 border border-zinc-100 dark:border-zinc-800 rounded-2xl shadow-2xl overflow-hidden py-2 p-1.5 ring-1 ring-black/5">
                <Link to="/guides" className="flex items-center gap-3 px-4 py-3 rounded-xl hover:bg-zinc-50 dark:hover:bg-zinc-800 group/item transition-colors" onClick={() => setIsGuidesOpen(false)}>
                  <div className="bg-primary/10 p-2 rounded-lg text-primary group-hover/item:bg-primary group-hover/item:text-zinc-900 transition-colors"><span className="material-icons text-sm">menu_book</span></div>
                  <span className="text-sm font-semibold dark:text-zinc-300">Investor Guides</span>
                </Link>
                <Link to="/inventory" className="flex items-center gap-3 px-4 py-3 rounded-xl hover:bg-zinc-50 dark:hover:bg-zinc-800 group/item transition-colors" onClick={() => setIsGuidesOpen(false)}>
                  <div className="bg-primary/10 p-2 rounded-lg text-primary group-hover/item:bg-primary group-hover/item:text-zinc-900 transition-colors"><span className="material-icons text-sm">table_view</span></div>
                  <span className="text-sm font-semibold dark:text-zinc-300">Property Inventory</span>
                </Link>
              </div>
            </div>
          </div>
          ...
          <div className="flex items-center gap-3 pl-4 border-l border-zinc-100 dark:border-zinc-800 h-6">
            ...
            <a href={`https://wa.me/${whatsappNumber}?text=${encodeURIComponent("Hi Yhen, I'm reaching out from the website.")}`} target="_blank" rel="noopener noreferrer"
              className="whatsapp-conversion w-8 h-8 rounded-lg flex items-center justify-center transition-all duration-300 hover:scale-110 hover:bg-[#25D366] group shadow-sm bg-zinc-50 dark:bg-zinc-800" title="WhatsApp"
              onClick={() => { window.dataLayer = window.dataLayer || []; window.dataLayer.push({ event: 'whatsapp_click', button_location: 'navbar_desktop' }); }}>
              <svg className="w-4 h-4 fill-zinc-400 group-hover:fill-white transition-colors pointer-events-none" viewBox="0 0 24 24">
      ...
      <div className={`md:hidden overflow-hidden transition-all duration-300 ${isMobileMenuOpen ? 'max-h-screen border-t border-zinc-200 dark:border-zinc-800' : 'max-h-0'}`}>
        <div className="px-4 py-4 space-y-2 bg-white dark:bg-zinc-900">
          ...
          <div>
            <button onClick={() => setIsGuidesOpen(!isGuidesOpen)}
              className="w-full flex items-center justify-between px-4 py-3 rounded-lg hover:bg-zinc-50 dark:hover:bg-zinc-800 transition-colors">
              <span className="font-semibold text-sm">Resources</span>
              <span className={`material-icons text-xs transition-transform ${isGuidesOpen ? 'rotate-180' : ''}`}>expand_more</span>
            </button>
            <div className={`overflow-hidden transition-all duration-200 ${isGuidesOpen ? 'max-h-32' : 'max-h-0'}`}>
              <div className="pl-4 space-y-1 mt-1">
                <Link to="/guides" className="flex items-center gap-3 px-4 py-2.5 rounded-lg hover:bg-zinc-50 dark:hover:bg-zinc-800" onClick={() => setIsMobileMenuOpen(false)}>
                  <span className="material-icons text-sm text-primary">menu_book</span><span className="text-sm">Investor Guides</span>
                </Link>
                <Link to="/inventory" className="flex items-center gap-3 px-4 py-2.5 rounded-lg hover:bg-zinc-50 dark:hover:bg-zinc-800" onClick={() => setIsMobileMenuOpen(false)}>
                  <span className="material-icons text-sm text-primary">table_view</span><span className="text-sm">Property Inventory</span>
                </Link>
              </div>
            </div>
          </div>
          ...
          <div className="flex items-center justify-center gap-3 pt-4 border-t border-zinc-200 dark:border-zinc-800">
            ...
            <a href={`https://wa.me/${whatsappNumber}?text=${encodeURIComponent("Hi Yhen, I'm reaching out from the website.")}`} target="_blank" rel="noopener noreferrer"
              className="whatsapp-conversion w-10 h-10 rounded-lg flex items-center justify-center transition-all hover:scale-110 hover:bg-[#25D366] group shadow-sm bg-zinc-50 dark:bg-zinc-800"
              onClick={() => { window.dataLayer = window.dataLayer || []; window.dataLayer.push({ event: 'whatsapp_click', button_location: 'navbar_mobile' }); }}>
              <svg className="w-5 h-5 fill-zinc-400 group-hover:fill-white transition-colors pointer-events-none" viewBox="0 0 24 24">
                <path d="M17.472 14.382c-.297-.149-1.758-.867-2.03-.967-.273-.099-.471-.148-.67.15-.197.297-.767.966-.94 1.164-.173.199-.347.223-.644.075-.297-.15-1.255-.463-2.39-1.475-.883-.788-1.48-1.761-1.653-2.059-.173-.297-.018-.458.13-.606.134-.133.298-.347.446-.52.149-.174.198-.298.298-.497.099-.198.05-.371-.025-.52-.075-.149-.669-1.612-.916-2.207-.242-.579-.487-.5-.669-.51-.173-.008-.371-.01-.57-.01-.198 0-.52.074-.792.372-.272.297-1.04 1.016-1.04 2.479 0 1.462 1.065 2.875 1.213 3.074.149.198 2.096 3.2 5.077 4.487.709.306 1.262.489 1.694.625.712.227 1.36.195 1.871.118.571-.085 1.758-.719 2.006-1.413.248-.694.248-1.289.173-1.413-.074-.124-.272-.198-.57-.347m-5.421 7.403h-.004a9.87 9.87 0 01-5.031-1.378l-.361-.214-3.741.982.998-3.648-.235-.374a9.86 9.86 0 01-1.51-5.26c.001-5.45 4.436-9.884 9.888-9.884 2.64 0 5.122 1.03 6.988 2.898a9.825 9.825 0 012.893 6.994c-.003 5.45-4.437 9.884-9.885 9.884m8.413-18.297A11.815 11.815 0 0012.05 0C5.495 0 .16 5.335.157 11.892c0 2.096.547 4.142 1.588 5.945L0 24l6.335-1.662c1.72.937 3.659 1.432 5.631 1.433h.005c6.554 0 11.89-5.335 11.893-11.893a11.821 11.821 0 00-3.48-8.413z"/>
              </svg>
            </a>
          </div>
